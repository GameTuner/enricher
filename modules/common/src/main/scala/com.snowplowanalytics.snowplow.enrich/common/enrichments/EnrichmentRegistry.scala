...
import com.snowplowanalytics.snowplow.enrich.common.enrichments.registry.uniqueid.{CreateUniqueIdWrapper, UniqueIdEnrichment}
...
object EnrichmentRegistry {
  ...
  // todo: ValidatedNel?
  def build[
    F[_]: Monad: CreateForex: CreateIabClient: CreateIpLookups: CreateOWM: CreateParser: CreateUaParser: sqlquery.CreateSqlQueryEnrichment: apirequest.CreateApiRequestEnrichment: CreateUniqueIdWrapper
  ](
    ...
    confs.foldLeft(EitherT.pure[F, String](EnrichmentRegistry[F]())) { (er, e) =>
      e match {
        ...
        case c: UniqueIdResolverConf =>
          for {
            enrichment <- EitherT.right(c.enrichment[F](blocker))
            registry <- er
          } yield registry.copy(uniqueIdResolver = enrichment.some)
      }
    }
      ...
      case _ =>
        (for {
          ...
          e = if (nm == "ip_lookups")
              ...
              else if (nm == "unique_id_resolver")
                UniqueIdEnrichment.parse(enrichmentConfig, schemaKey).map(_.some)
              else
...
final case class EnrichmentRegistry[F[_]](
  ...
  uniqueIdResolver: Option[UniqueIdEnrichment[F]] = None,
)
