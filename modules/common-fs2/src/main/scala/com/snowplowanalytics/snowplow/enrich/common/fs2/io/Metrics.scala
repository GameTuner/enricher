...
object Metrics {
  ...
  def stdoutReporter[F[_]: Sync: ContextShift: Timer](
    ...
    for {
      ...
      private val latencyMeasure = MeasureLong.create(
        "gametuner_enricher/enrich_latency",
        "Latency of event processing", "ms")

      private val view: View = View.create(
        Name.create("gametuner_enricher/enrich_latency"),
      ...
      def report(snapshot: MetricSnapshot): F[Unit] =
        for {
          ...
          _ <- snapshot.enrichLatency
                 .map(latency => {
                   Stats.getStatsRecorder.newMeasureMap().put(latencyMeasure, latency).record();
                   logger.info(s"${MetricsReporters.normalizeMetric(config.prefix, LatencyGaugeName)} = $latency")
                 })
