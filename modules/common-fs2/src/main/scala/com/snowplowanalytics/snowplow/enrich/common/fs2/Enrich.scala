...
import com.snowplowanalytics.snowplow.enrich.common.{EtlPipeline, GcpMetrics, MetadataClient}
import com.snowplowanalytics.snowplow.enrich.common.outputs.EnrichedEvent
import com.snowplowanalytics.snowplow.enrich.common.adapters.AdapterRegistry
import com.snowplowanalytics.snowplow.enrich.common.loaders.{CollectorPayload, ThriftLoader}
import com.snowplowanalytics.snowplow.enrich.common.enrichments.{BadRowMetrics, EnrichmentRegistry}
...
object Enrich {
  ...
  def run[F[_]: Concurrent: ContextShift: Clock: HttpClient: Parallel: Timer, A](env: Environment[F, A]): Stream[F, Unit] = {
    MetadataClient.startDataUpdate(env.metadataService.appIdConfigsEndpoint, env.metadataService.cacheRefreshIntervalSeconds)
    GcpMetrics.enable()
  ...
  def badRowResize[F[_], A](env: Environment[F, A], badRow: BadRow): Array[Byte] = {
    ...
    var finalBadRow = badRow
    val processedBadRow = if (size > maxRecordSize) {
    ...
    val eventErrorDetails = finalBadRow match {
      case BadRow.SchemaViolations(_, _, payload) => BadRowMetrics.eventFailureDetails(
        payload.enriched, BadRow.SchemaViolations.getClass.getSimpleName)
      case BadRow.EnrichmentFailures(_, _, payload) => BadRowMetrics.eventFailureDetails(
        payload.enriched, BadRow.EnrichmentFailures.getClass.getSimpleName)
      case other: BadRow => BadRowMetrics.EventFailureDetails("unknown", "unknown", other.getClass.getSimpleName)
    }
    BadRowMetrics.trackEventError(eventErrorDetails)
